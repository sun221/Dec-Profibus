...
def setTokenStart(masterList):
	"""set the token Start"""
	masterList[0].haveToken = False					#does the master starts with token or not?
...
def goNextToken(masterList): 
	...
	masterList[0]._DpMaster__sendToMasterAdd(1)
...
try:
	...
	config = dec_profibus.PbConf.fromFile("master/conf_file.conf")
	config1 = dec_profibus.PbConf.fromFile("master/conf_file1.conf")
		...
		for master in masterList:
			for slaveDesc in master.getSlaveList():
				if slaveDesc.slaveAddr in outDataList[masterList.index(master)].keys():
					...
					outDataList[masterList.index(master)][slaveDesc.slaveAddr] = outDataList[masterList.index(master)][60]
					slaveDesc.setOutData(outDataList[masterList.index(master)][slaveDesc.slaveAddr])
