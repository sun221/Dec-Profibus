...
from dec_profibus.layers.dec_scapy import *
from dec_profibus.layers.DpLayer import *
from dec_profibus.layers.auto_response import *
...
class DpSlaveState(object):
	...
	STATE_WADD	= 2
	STATE_WPRM	= 3 # Wait for Prm telegram
	STATE_WCFG	= 4 # Wait for Cfg telegram
	STATE_WDXRDY	= 5 # Wait for Data_Exchange ready
	STATE_DX	= 6 # Data_Exchange

	state2name = {
		_STATE_INVALID	: "invalid",
		STATE_INIT	: "init",
		STATE_WDIAG	: "wait for diag",
		STATE_WADD	: "wait for address",
	...
	def __init__(self, master, slaveDesc):
		...
		self.__savedState = self.STATE_WDIAG				#here you can choose the first to go ex : self.__savedState = self.STATE_INIT 
	...
	def saveState(self):
		self.__savedState = self.__state
		return self.__savedState
	def getSave(self):
		return self.__savedState

	def restore(self):
		self.__state = self.__savedState
		self.__prevState = self.__state - 1 
	...
	def setState(self, state, stateTimeLimit=None):
		...
		if(self.master.phy is not None and not isinstance(self.master.phy,str)):
...
class DpSlaveDesc(object):
	...
	def __init__(self,
		...
		#Prepare a Set_Add telegram
		self.setAddTelegram = DpTelegram_SET_SLAVE_ADDR(DA = self.slaveAddr, SA=None)
		self.setAddTelegram.newAdd = 42
		self.setAddTelegram.identNumber = self.identNumber
		self.setAddTelegram.setDU()
...
class DpMaster(object):
	def __init__(self, dpmClass, phy, masterAddr, debug=False ,sock_mod = True , dataBox=True ,useUdp = True):
		self.dpmClass = dpmClass
		self.phy = phy
		self.masterAddr = masterAddr
		self.debug = debug
		#socket mod parameters
		self.sock_mod = sock_mod
		self.dataBox = dataBox
		self.useUdp = useUdp

		# Create the transceivers
		self.fdlTrans = FdlTransceiver(self.phy ,sock_mod = self.sock_mod , dataBox=self.dataBox  ,useUdp = self.useUdp)
		self.dpTrans = DpTransceiver(self.fdlTrans, thisIsMaster=True ,sock_mod = self.sock_mod , dataBox=self.dataBox  ,useUdp = self.useUdp)

		mcastSlaveDesc = DpSlaveDesc(index="Mcast",gsd=None,
					     slaveAddr=FdlSd1.ADDRESS_MCAST)
		mcastSlave = DpSlaveState(self, mcastSlaveDesc)

		self.__slaveDescs = DictList()
		self.__slaveDescs[FdlSd1.ADDRESS_MCAST ] =  mcastSlaveDesc
		
		self.__slaveStates = DictList()
		self.__slaveStates[FdlSd1.ADDRESS_MCAST ] = mcastSlave
	...
	def __debugMsg(self, msg):
		if self.debug:
			print("DPM%d:%d %s" % (self.dpmClass, self.masterAddr,msg))

	def __errorMsg(self, msg):
		print("DPM%d:%d  >ERROR<  %s" % (self.dpmClass , self.masterAddr, msg))
	...
	def destroy(self):
		if self.phy and not isinstance(self.phy,str):
			self.phy.close()
			self.phy = None

	def addSlave(self, slaveDesc ,slaveState = None):
		"""Register a slave."""
		...
		if slaveState is None:
			...
			self.__slaveStates[slaveAddr] = slaveState
		...
		self.__slaveDescsList = [
			...
			for addr, desc in sorted(self.__slaveDescs.items(),
						 key=lambda x: x[0])
			if addr != FdlSd1.ADDRESS_MCAST
		]

		#self.__runNextSlaveIndex = 0

	
	def removeSlave(self, slaveAddr):
		"""Register a slave."""

		
		if slaveAddr not in self.__slaveDescs or\
		   slaveAddr not in self.__slaveStates:
			raise DpError("Slave %d is none existant" % slaveAddr)
		self.__slaveDescs[slaveAddr] = None
		self.__slaveStates[slaveAddr] = None
		del self.__slaveDescs[slaveAddr]
		del self.__slaveStates[slaveAddr] 
	...
	#modify
	def modifySlave(self,slaveAddr, slaveIndex,newAddr ):
		""" Modify a slave."""
		...
		if slaveAddr not in self.__slaveDescs or\
		   slaveAddr not in self.__slaveStates:
			raise DpError("Slave %d is none existant" % slaveAddr)
		if slaveAddr == 126 :
			if isinstance(self.__slaveDescs.get(126),list) and isinstance(self.__slaveStates.get(126),list):  #many none set addresses 126s
				for idx ,slaveDes in enumerate(self.__slaveDescs.get(126)):
					if slaveDes.index == slaveIndex:
						Desc = slaveDes
						id=idx
						Desc.slaveAddr = newAddr
						self.addSlave(Desc ,self.__slaveStates[slaveAddr] )
						break
				self.__slaveDescs.get(126).pop(id)
				for idx ,slaveState in enumerate(self.__slaveStates.get(126)):
					if slaveState.slaveDesc.index == slaveIndex :
						id=idx
						break
				self.__slaveStates.get(126).pop(id)

			else:
				Desc = self.__slaveDescs[slaveAddr] 
				Desc.slaveAddr = newAddr
				self.addSlave(Desc ,self.__slaveStates[slaveAddr] )
				del self.__slaveDescs[slaveAddr]
				del self.__slaveStates[slaveAddr] 
			...
			del self.__slaveDescs[slaveAddr]
			del self.__slaveStates[slaveAddr] 
		from dec_profibus.master.conf import PbConf
		PbConf.writeAdd(slaveIndex,newAddr , "conf_file.conf")
		...
		return self.__slaveStates[newAddr]
	...
	def __send(self, slave, telegram, timeout):
		...
		return True
	def __sendToMaster(self, master ,tel = None ):
		"""Asynchronously send a telegram to a master.
		"""
		try:
			for state in self.__slaveStates:
				self.__slaveStates[state].saveState()
			tr = self.fdlTrans
			if tel == None:
				tr.sendNoFcb(telegram=FdlTelegram_token(master.masterAddr,self.masterAddr))
			else:
				tr.sendNoFcb(telegram=tel)
		except PhyError as e:
			self.__debugMsg(str(e))
			return False
		return True
	def __sendToMasterAdd(self, add ,tel = None ):
		"""Asynchronously send a telegram to a master.
		"""
		try:
			for state in self.__slaveStates:
			...
			tr = self.fdlTrans
			if tel == None:
				tr.sendNoFcb(telegram=FdlTelegram_token(add,self.masterAddr))
			else:
				tr.sendNoFcb(telegram=tel)
		except PhyError as e:
			self.__debugMsg(str(e))
	...
	def _releaseSlave(self, slave):
		if self.phy is not None and not isinstance(self.phy,str):
			self.phy.releaseBus()
		...
		if slave.stateJustEntered():
			...
			for telegram in slave.getRxQueue():
				if telegram.FC is not None:
					slave.pendingReq = None
					stype = telegram.FC & FdlSd1.FC_STYPE_MASK
					if telegram.FC & FdlSd1.FC_REQ:
						self.__debugMsg("Slave %d replied with "
								"request bit set." %\
								slave.slaveDesc.slaveAddr)
					elif stype != FdlSd1.FC_SLAVE:
		...
		if slave.stateJustEntered():
			...
			for telegram in slave.getRxQueue():
				if DpTelegram_SlaveDiag_Con.checkType(telegram):
					slave.setState(slave.STATE_WADD)
					return None
				else:
	...
	def __runSlave_verifyAdd(self, slave):
		if slave.stateJustEntered():
			self.__debugMsg("Making sure Slave have valid address %d..." %\
				slave.slaveDesc.slaveAddr)
			slave.flushRxQueue()
		elif slave.slaveDesc.slaveAddr != 126 and slave.slaveDesc.slaveAddr != 60:   #60 can be changed
			self.__debugMsg("valid adress")
			slave.setState(slave.STATE_WPRM)
			return None
		elif slave.shortAckReceived:
			if slave.slaveDesc.slaveAddr == 126:
				self.modifySlave(126,slave.slaveDesc.index,slave.slaveDesc.setAddTelegram.newAdd)
			if slave.slaveDesc.slaveAddr == 60:  		#60 can be changed
				slave = self.modifySlave(60,slave.slaveDesc.index,slave.slaveDesc.setAddTelegram.newAdd)
			slave.fcb.handleReply()
			slave.setState(slave.STATE_WPRM)
			return None


		elif (not slave.pendingReq or
		    slave.pendingReqTimeout.exceed() and slave.slaveDesc.slaveAddr == 126):
			# Send a Set_Prm request
			slave.slaveDesc.setAddTelegram.SA = self.masterAddr
			ok = self.__send(slave,
					 telegram=slave.slaveDesc.setAddTelegram,
					 timeout=0.05)
			if not ok:
	...
	def __runSlave_waitPrm(self, slave):
		...
		if (not slave.pendingReq or
			...
			slave.slaveDesc.setPrmTelegram.DA = slave.slaveDesc.slaveAddr
			ok = self.__send(slave,
	...
	def __runSlave_waitCfg(self, slave):
		...
		if (not slave.pendingReq or
			...
			slave.slaveDesc.chkCfgTelegram.DA = slave.slaveDesc.slaveAddr
		...
		if slave.pendingReq:
			for telegram in slave.getRxQueue():
				if not DpTelegram_DataExchange_Con.checkType(telegram):
					...
					continue
				resFunc = telegram.FC & FdlSd1.FC_RESFUNC_MASK
				if resFunc in {FdlSd1.FC_DH,
					       FdlSd1.FC_RDH}:
					self.__debugMsg("Slave %d requested diagnostics." %\
						slave.slaveDesc.slaveAddr)
					slave.setState(slave.STATE_WDXRDY, 0.2)
				elif resFunc == FdlSd1.FC_RS:
	...
	__slaveStateHandlers = {
		...
		DpSlaveState.STATE_WADD		: __runSlave_verifyAdd,
	...
	def __pollRx(self):
		try:
			...
			return
		if ok and telegram:
			if telegram.FC == 0x49 and telegram.DA == self.masterAddr:	#Initialization check
					fdlTelegram = FdlTelegram_FdlStat_Con(DA = telegram.SA,
										SA = telegram.DA)
					fdlTelegram.FC = 0x20
					self.__sendToMasterAdd(telegram.SA,fdlTelegram)
					return
			if isinstance(telegram,FdlSd1) and telegram.SD == 0xDC and telegram.DA == self.masterAddr:
				print("got the token")
				self.haveToken= True 
				for addr in self.__slaveStates:
					self.__slaveStates[addr].restore()
					self.__slaveStates[addr].stateJustEntered()
					self.__sendToMasterAdd(telegram.SA,FdlTelegram_ack())

				
			elif isinstance(telegram,FdlSd1) and telegram.SD == FdlSd1.SC:
				for addr, slave in self.__slaveStates.items():
					if addr != FdlSd1.ADDRESS_MCAST:
						slave.shortAckReceived = True
			elif telegram.DA == FdlSd1.ADDRESS_MCAST:
	...
	def initialize(self):
		...
		self.fdlTrans.setRXFilter([self.masterAddr,
					   FdlSd1.ADDRESS_MCAST])
	def diagnostic(self):
		for addr in self.__slaveStates:
			self.__slaveStates[addr].setState(1)
			#self.__slaveStates[addr].stateJustEntered()			

	def __syncFreezeHelper(self, groupMask, controlCommand):
		slave = self.__slaveStates[FdlSd1.ADDRESS_MCAST]
		globCtl = DpTelegram_GlobalControl(DA = FdlSd1.ADDRESS_MCAST,
						   SA = self.masterAddr)
...
class DPM1(DpMaster):
	def __init__(self, phy, masterAddr, debug=False ,sock_mod = True , dataBox=True ,useUdp = True):
		self.__haveToken = True
		DpMaster.__init__(self, dpmClass=1, phy=phy,
			masterAddr=masterAddr,
			debug=debug ,sock_mod = sock_mod , dataBox=dataBox ,useUdp = useUdp)
