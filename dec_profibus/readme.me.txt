...
Phy Layer is the underlaying layer that all other physical layers should be built on, it has many methods that must be overwrited  :
...
socket , serial and Pcap Layers are built on the Phy Layer and overwite all its methods  :
the most important ones are : 
- Close(self) : specify how the physical connexion should be closed and killed .
- pollData(timeout)   : specify how the data gonna be polled and readed from the physical bus .
- sendData(telegramData , srd) : specify how the data should be sended through the physical Bus , telegramData is the raw data that's gonna be transmitted through the bus .
...
```sh
 ...
 raw(telegramData)   : transform the packet to raw data
```
...
Use example :
```sh
 fdlTr = FdlTranseiver( None , sock_mod = True)  # socket mod
  fdlTr = FdlTranseiver( physerial , sock_mod = False)  # serial mod
```
##### Fdl Default Packet  
Transeiver tha can trasnmit Profibus-DP (Layer 2 FDL ) formated Data ,
- FdlTelegram_stat0  : SD1 packet
- FdlTelegram_token  : SD4 Packet
- FdlTelegram_ack   : Ack packet 
- FdlTelegram_var  : Varuable DU packet
- FdlTelegram_stat8 : SD3 Packet
